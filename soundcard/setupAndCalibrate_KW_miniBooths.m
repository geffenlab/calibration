function [FILT, fs] = setupAndCalibrate_KW_miniBooths
...
io.h = PsychPortAudio('Open', [], 3, 3, io.fs, [2 1]);
...
[resp, P, f, dB] = getResponse_sess_KWppiSC(stim,n,io);
resp = mean(resp);
figure(1); hold on
plot(f,dB,'r');
...
[resp, P, f, dB] = getResponse_sess_KWppiSC(noisef,n,io);
plot(f,dB);
...
[RMS, dBs] = toneResponse_KWppiSC(toneio.freqs, .1 * softGain, 1, FILT, io);
toneDB = real( 20*log10(RMS) );
plot(toneio.freqs, toneDB, 'ok');
hold off

%
% save('C:\Users\geffen-behaviour2\Documents\MATLAB\miniBooth2Calib\20170109_upperBoothRightSpkrInvFilt_3k-80k_fs192k.mat','FILT_RIGHT')